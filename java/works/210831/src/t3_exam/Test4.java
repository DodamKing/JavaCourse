...
public class Test4 {
	public static void main(String[] args) {
		...
		while (true) {
			...
			for (int i=0; i<3; i++) {
				System.out.printf("%s점수 입력(종료999): ", title[i+1]); //국영수 넣기
		...
		// 최고총점
//		for (int i=0; i<cnt; i++) if (max < jumsu[i][3]) max = jumsu[i][3]; 
		for (int i=0; i<cnt; i++) if (jumsu[i][4] == 1) max = jumsu[i][3]; // 새로운 방법을 떠올려 봄
		
		// 총평균
		for (int i=0; i<cnt; i++) t_avg += (avg[i] / cnt);
